...
Regression on CPU (`make test-cpp`) uses **one** heap arena for all 73 cases (`BeginRegressionArena` / `EndRegressionArena`), resetting the bump offset between cases instead of malloc/free per case.
