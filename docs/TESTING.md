...
make test-cpp     # ./netkit test only (89 embedded .nk cases)
...
Both `make test-cpp` and `make test-c` exercise the **same 89 embedded cases** via `run_all_tests()` / `nk_run_all_tests()`:
...
**Total: 89 passed** when healthy (`19` hand + `10` MNIST MLP + `10` MNIST CNN + `17` op matrix + `27` ONNX import extensions + `1` MobileNetV4 Small + `1` MobileNetV4 Small int8 + `2` YOLOX (full PAFPN + synthetic taps) + `1` ResNet-18 + `1` ConvNeXt V2-Atto).
...
|---------|:----------:|--------|----------------|
| `make test` / `make test-cpp` / `make test-c` | Yes | hand + MNIST + op-matrix + import extensions + MobileNetV4 / YOLOX / ResNet-18 / ConvNeXt V2 backbones | C++/C `.nk` load + forward vs embedded TCAS (**89 cases**) + fast Python (AOT, unit tests) |
...
| Full regression | `nk_run_all_tests()` — same **89** embedded cases as C++ (`run_all_tests()` in `src/test.cpp`) |
