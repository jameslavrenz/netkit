// NUCLEO-F446RE microTVM AOT MNIST DS-CNN int8 benchmark — same 10 images as netkit/TFLM.
...
#include "tvmgen_mnist_cnn_dw_int8.h"
...
{
    uart_printf(
        "DIGIT_SUMMARY runtime=tvm model=cnn_dw_int8 image=%d label=%d pred=%d pred_i8=%d ok=%d out_i8=",
        image,
        label,
        predicted,
        pred_i8,
...
int InvokeOnce()
{
    struct tvmgen_mnist_cnn_dw_int8_inputs inputs = {};
    struct tvmgen_mnist_cnn_dw_int8_outputs outputs = {};
    NETKIT_TVM_BIND_INPUT(inputs, g_input);
    NETKIT_TVM_BIND_OUTPUT(outputs, g_output);
    return tvmgen_mnist_cnn_dw_int8_run(&inputs, &outputs);
}
...
extern "C" int main(void)
{
    ...
    uart_write("\r\nTVM NUCLEO-F446RE MNIST DS-CNN int8 benchmark\r\n");
...
#define TVM_MCU_BACKEND_LABEL "cmsis-nn"
    ...
    uart_printf("\r\nTVM MNIST cnn_dw int8 benchmark summary (microTVM AOT %s)\r\n",
                TVM_MCU_BACKEND_LABEL);
    uart_printf("  method:      %d runs x 10 images, discard first invoke each run\r\n", kRuns);
    uart_printf("  mean:   %8lu us (%6lu ms)\r\n", mean_us_i, mean_ms_i);
    uart_printf(
        "BENCHMARK_SUMMARY runtime=tvm model=cnn_dw_int8 backend=%s mean_us=%lu runs=%d\r\n",
        TVM_MCU_BACKEND_LABEL,
        mean_us_i,
        kRuns);
    uart_write("\r\nDONE\r\n");
