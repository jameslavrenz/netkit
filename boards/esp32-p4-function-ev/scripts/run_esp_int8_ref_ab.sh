#!/usr/bin/env bash
# ESP32-P4-Function-EV int8 peer A/B — ESP-NN OFF (reference) for netkit + TFLM.
#
# Same MCU methodology as ESP-NN-on A/B: 10 runs × 10 images, discard first
# invoke each run; order swaps nk→tflm and tflm→nk.
#
# Usage:
#   PORT=/dev/cu.usbmodem21101 ./boards/esp32-p4-function-ev/scripts/run_esp_int8_ref_ab.sh
#   MODELS="cnn" TIMEOUT=900 ./boards/esp32-p4-function-ev/scripts/run_esp_int8_ref_ab.sh
set -euo pipefail
...
TIMEOUT="${TIMEOUT:-900}"
BAUD="${BAUD:-115200}"
PIO_ENV="${PIO_ENV:-esp32_p4_ev_ref}"
