// NUCLEO-F446RE MNIST DS-CNN int8 invoke benchmark — same 10 images as benchmark/netkit.
// Supports interpreter embed (default) or quant lowered embed (NETKIT_LOWERED=1).
// Test inputs are prequantized int8 (cnn_dw/mnist_cnn_int8_test_images.*) — no float conversion.
...
#else
#define NK_CNN_DW_INT8_BACKEND_LABEL "cmsis-nn-int8"
#endif
...
// STM32F446RE has 128 KiB SRAM — DS-CNN int8 needs more than the 64 KiB CNN
// default; board Makefile sets NETKIT_ARENA_KB=96.
...
extern "C" int main(void)
{
    ...
    uart_write("\r\nnetkit NUCLEO-F446RE MNIST DS-CNN int8 benchmark\r\n");
    ...
    uart_write("\r\nnetkit MNIST cnn_dw int8 benchmark summary (");
    ...
    uart_printf("  mean:   %lu us (%lu.%03lu ms)\r\n",
                ...
                mean_ms_frac);
    uart_printf(
        "BENCHMARK_SUMMARY runtime=netkit model=cnn_dw_int8 backend=" NK_CNN_DW_INT8_BACKEND_LABEL
        " mean_us=%lu runs=%d\r\n",
        mean_us_i,
        kRuns);
#if defined(NETKIT_STAGE_TIMING) && NETKIT_STAGE_TIMING
