// Seeed XIAO ESP32S3 — MNIST CNN int8 (netkit ESP-NN, interpreter embed).
...
#else
#define NK_CNN_INT8_BACKEND_LABEL "esp-nn-int8"
#endif
...
// AOT ~90 KiB; S3 ESP-NN asm scratch can push peak higher — match TFLM peer (96 KiB).
constexpr std::size_t kArenaCapacity =
    (aot::kArenaBytesRecommended > (96u * 1024u)) ? aot::kArenaBytesRecommended : (96u * 1024u);
...
{
    std::printf("\nnetkit XIAO ESP32S3 MNIST CNN int8 benchmark\n");
    std::printf("  backend:     %s (MCU ESP32S3, interpreter embed)\n",
    ...
    std::printf("\nnetkit MNIST cnn int8 benchmark summary (%s)\n", NK_CNN_INT8_BACKEND_LABEL);
    ...
    std::printf("BENCHMARK_SUMMARY runtime=netkit model=cnn_int8 backend=" NK_CNN_INT8_BACKEND_LABEL
                " mean_us=%lu runs=%d\n",
                mean_us_i,
                kRuns);
    std::printf("\nDONE\n");
