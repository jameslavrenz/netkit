// Seeed XIAO ESP32C3 — MNIST MLP int8 invoke benchmark (same 10 images as NUCLEO).
// Profile: NETKIT_TARGET=mcu_esp + NETKIT_ARCH=ESP32C3 + ESP-NN (RISC-V silicon).
...
#else
#define NK_MLP_INT8_BACKEND_LABEL "esp-nn-int8"
#endif
...
extern "C" void app_main(void)
{
    ...
    std::printf("  nk bytes:    %u\n", static_cast<unsigned>(aot::kNkBytes));
    std::printf("  sysclk:      160000000 Hz (ESP32-C3 typical)\n");
    ...
    std::printf("\nnetkit MNIST mlp int8 benchmark summary (%s)\n", NK_MLP_INT8_BACKEND_LABEL);
    ...
    std::printf("BENCHMARK_SUMMARY runtime=netkit model=mlp_int8 backend=" NK_MLP_INT8_BACKEND_LABEL
                " mean_us=%lu runs=%d\n",
                mean_us_i,
                kRuns);
    std::printf("\nDONE\n");
