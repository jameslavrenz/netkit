// Seeed ESP32-P4-Function-EV — MNIST DS-CNN float32 (netkit reference, lowered AOT).
// Methodology: 10 runs × 10 images, discard image 0 each run. ESP-NN has no float API.

#include "esp_heap_caps.h"
#include "esp_timer.h"
#include "mnist_cnn_dw_aot.hpp"
...
namespace aot = netkit::aot::mnist_cnn_dw;
...
{
    std::printf(
        "DIGIT_SUMMARY runtime=netkit model=cnn_dw_f32 image=%d label=%d pred=%d pred_f=%.4f ok=%d out_f=",
...
{
    std::printf("\nnetkit ESP32-P4-Function-EV MNIST DS-CNN float32 benchmark\n");
    std::printf("  backend:     netkit reference (MCU ESP32P4 FPU, lowered AOT)\n");
    ...
    std::printf("\nnetkit MNIST cnn_dw_f32 float32 benchmark summary (reference)\n");
    ...
    std::printf("BENCHMARK_SUMMARY runtime=netkit model=cnn_dw_f32 backend=reference-f32 mean_us=%lu runs=%d\n",
                mean_us_i, kRuns);
    std::printf("\nDONE\n");
}
