// Seeed XIAO ESP32S3 — TFLM MNIST DS-CNN float32.
// Same 10×10 methodology as int8 peers. ESP-NN unused (no float kernels).

#include "esp_heap_caps.h"
#include "esp_timer.h"
#include "mnist_cnn_dw_model_data.h"
...
{
    std::printf(
        "DIGIT_SUMMARY runtime=tflm model=cnn_dw_f32 image=%d label=%d pred=%d pred_f=%.4f ok=%d out_f=",
...
extern "C" void app_main(void)
{
    ...
    const tflite::Model* model = tflite::GetModel(g_mnist_cnn_dw_model_data);
    ...
    std::printf("\nTFLM XIAO ESP32S3 MNIST DS-CNN float32 benchmark\n");
    std::printf("  backend:     tflm reference float32 (MCU ESP32S3 FPU)\n");
    std::printf("  model bytes: %u\n", g_mnist_cnn_dw_model_data_size);
    ...
    std::printf("\nTFLM MNIST cnn_dw_f32 float32 benchmark summary (reference)\n");
    std::printf("  mean:   %lu us (%lu.%03lu ms)\n", mean_us_i, mean_ms_whole, mean_ms_frac);
    std::printf("BENCHMARK_SUMMARY runtime=tflm model=cnn_dw_f32 backend=reference-f32 mean_us=%lu runs=%d\n",
                mean_us_i, kRuns);
    std::printf("\nDONE\n");
}
