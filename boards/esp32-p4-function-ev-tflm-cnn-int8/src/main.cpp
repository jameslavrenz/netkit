...
#if defined(ESP_NN)
#define TFLM_CNN_INT8_BACKEND_LABEL "esp-nn"
#else
#define TFLM_CNN_INT8_BACKEND_LABEL "reference"
#endif

constexpr int kRuns = 10;
constexpr int kOutputClasses = 10;
// ESP32-P4 SRAM is tight with IDF; 96 KiB matches published NUCLEO A/B arena budget.
...
extern "C" void app_main(void)
{
    ...
    std::printf("\nTFLM ESP32-P4-Function-EV MNIST CNN int8 benchmark\n");
    std::printf("  backend:     tflm %s int8 (MCU ESP32P4)\n", TFLM_CNN_INT8_BACKEND_LABEL);
    ...
    std::printf("\nTFLM MNIST cnn int8 benchmark summary (%s)\n", TFLM_CNN_INT8_BACKEND_LABEL);
    ...
    std::printf("BENCHMARK_SUMMARY runtime=tflm model=cnn_int8 backend=%s mean_us=%lu runs=%d\n",
                TFLM_CNN_INT8_BACKEND_LABEL,
                mean_us_i,
                kRuns);
    std::printf("\nDONE\n");
