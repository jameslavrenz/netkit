/*
 * NMSIS-NN int8 adapters for netkit quantized conv / pool / FC (TFLite-style).
 */
...
namespace
{
    bool nmsis_status_ok(riscv_nmsis_nn_status status)
    {
        return status == RISCV_NMSIS_NN_SUCCESS;
    }
    ...
    nmsis_nn_activation activation_clamp(QuantInteger::QuantClamp clamp,
                                         float output_scale,
    ...
    nmsis_nn_activation activation_relu(bool apply_relu)
    {
    ...
    bool BindContext(nmsis_nn_context& ctx, int32_t required_bytes)
    {
        if (required_bytes <= 0)
        {
            ctx.buf = nullptr;
                    ...
                    Arena& arena)
{
    ...
    if (weights && bias && plan.workspace_bytes > 0 && !plan.kernel_sums)
    {
        ...
        if (riscv_vector_sum_s8(plan.kernel_sums,
                              plan.in_features,
                              plan.out_features,
                              weights,
                              plan.input_offset,
                              plan.filter_offset,
                              bias) != RISCV_NMSIS_NN_SUCCESS)
        {
            return false;
        }
    }
                            ...
                            int8_t* output)
{
...
#endif
#endif
    const bool cmsis_ok = nmsis_status_ok(riscv_convolve_wrapper_s8(&ctx,
                                                 &plan.cmsis.conv,
                                                 &plan.cmsis.quant,
                                                 &plan.cmsis.input,
                                                 input,
    ...
    const nmsis_nn_dims bias_dims = {.n = 1, .h = 1, .w = 1, .c = plan.out_c};
    const nmsis_nn_dims output_dims = {
        .n = 1, .h = plan.out_h, .w = plan.out_w, .c = plan.out_c,
    };

    if (!nmsis_status_ok(riscv_convolve_wrapper_s8(&ctx,
                                                 &conv_params,
                                                 &quant_params,
                                                 &input_dims,
                                                 input,
                                     ...
                                     int8_t* output)
{
...
#if NETKIT_CMSIS_PLAN_HOIST
    if (!nmsis_status_ok(riscv_depthwise_conv_wrapper_s8(&ctx,
                                                       &plan.cmsis.dw_conv,
                                                       &plan.cmsis.quant,
                                                       &plan.cmsis.input,
                                                       input,
    ...
    const nmsis_nn_dims bias_dims = {.n = 1, .h = 1, .w = 1, .c = plan.channels};
    const nmsis_nn_dims output_dims = {
                               ...
                               int8_t* output)
{
    ...
    nmsis_nn_context ctx = {0};
#if NETKIT_CMSIS_PLAN_HOIST
    if (!nmsis_status_ok(riscv_max_pool_s8(&ctx,
                                         &plan.cmsis.pool,
                                         &plan.cmsis.input,
                                         input,
                                         &plan.cmsis.filter,
    ...
    if (!nmsis_status_ok(
            riscv_max_pool_s8(&ctx, &pool_params, &input_dims, input, &filter_dims, &output_dims, output)))
#endif
    {
        return false;
    }
                                ...
                                int8_t* output_int8)
{
    ...
    {
        if (riscv_vector_sum_s8(static_cast<int32_t*>(ctx.buf),
                              plan.in_features,
                              plan.out_features,
                              weights,
                              plan.input_offset,
    ...
    const bool per_channel =
        plan.weight_channel_scales != nullptr &&
        plan.num_weight_channel_scales == static_cast<uint32_t>(plan.out_features);
    const nmsis_nn_quant_params wrapper_quant = {
        .multiplier = plan.multipliers,
        .shift = plan.shifts,
        .is_per_channel = per_channel ? 1 : 0,
    };
...
#if NETKIT_CMSIS_PLAN_HOIST
    if (!nmsis_status_ok(riscv_fully_connected_wrapper_s8(&ctx,
                                                        &plan.cmsis.fc,
                                                        &wrapper_quant,
                                                        &plan.cmsis.input,
                                                        input,
    ...
    const nmsis_nn_dims input_dims = {.n = 1, .h = 1, .w = 1, .c = plan.in_features};
    const nmsis_nn_dims filter_dims = {
        .n = plan.in_features, .h = 1, .w = 1, .c = plan.out_features,
    };
    const nmsis_nn_dims bias_dims = {.n = 1, .h = 1, .w = 1, .c = plan.out_features};
    const nmsis_nn_dims output_dims = {.n = 1, .h = 1, .w = 1, .c = plan.out_features};
                        ...
                        int8_t* output)
{
    ...
    const nmsis_nn_dims bias_dims = {.n = 1, .h = 1, .w = 1, .c = out_channels};
    const nmsis_nn_dims output_dims = {
        .n = 1,
                                 ...
                                 int8_t* output)
{
    ...
    const nmsis_nn_dims bias_dims = {.n = 1, .h = 1, .w = 1, .c = static_cast<int32_t>(channels)};
    const nmsis_nn_dims output_dims = {
        .n = 1,
        .h = static_cast<int32_t>(out_h),
        .w = static_cast<int32_t>(out_w),
                         ...
                         int8_t* output)
{
    ...
    QuantizeMultiplier(
        ...
        &output_mult,
        &output_shift);
    const riscv_nmsis_nn_status status = riscv_elementwise_add_s8(
        input1,
        input2,
        -input1_zero_point,
        input1_mult,
