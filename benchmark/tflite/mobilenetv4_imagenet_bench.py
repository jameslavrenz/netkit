#!/usr/bin/env python3
"""Host TensorFlow Lite (LiteRT) float32 ImageNet MobileNetV4-Conv-Small bench.

Pairs with benchmark/netkit/src/mobilenetv4_imagenet_main.cc:
  - same pretrained .tflite (benchmark/tflm/generated/mobilenetv4_imagenet_f32.tflite)
  - same 10 ImageNet-preprocessed images (10 distinct classes)
  - same methodology (10 images x 5 loops, report top-1 + 10-image mean)

This is the MPU-oriented TF Lite interpreter (ai-edge-litert / former tflite-runtime),
not TFLM. XNNPACK is the default CPU delegate on host — that is the path Google
typically recommends for MPU/desktop, analogous to CMSIS-NN on MCU.
"""
...
DEFAULT_MODEL = ROOT / "benchmark" / "tflm" / "generated" / "mobilenetv4_imagenet_f32.tflite"
...
def _load_images() -> list[tuple[str, int, np.ndarray]]:
    ...
    for filename, _url, label, short in SAMPLES:
        ...
        pixels = preprocess(rgb)  # NHWC float32 224x224x3
        out.append((short, label, pixels))
    return out
...
def main() -> int:
    ...
    if not args.model.is_file():
        raise SystemExit(
            f"missing {args.model} — run: make -C benchmark/tflm export-mobilenetv4-imagenet"
        )
    ...
    print("TF Lite MobileNetV4 ImageNet float32 benchmark")
    print(f"  runtime:     TensorFlow Lite / LiteRT (MPU interpreter)")
    ...
    for loop in range(args.loops):
        for i, (short, label, pixels) in enumerate(images):
            batch = pixels.reshape(1, 224, 224, 3).astype(np.float32, copy=False)
    ...
    print()
    print(f"TF Lite MobileNetV4 ImageNet summary ({backend})")
    ...
    print(
        "BENCHMARK_SUMMARY runtime=tflite model=mobilenetv4_imagenet dtype=float32 "
