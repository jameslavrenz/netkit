// netkit MNIST MLP per-layer invoke profiler (pairs with benchmark/tflm/).

#include "arena.hpp"
#include "arena_util.hpp"
#include "benchmark_invoke_profile.hpp"
...
{
    BenchmarkInvokeProfile::TagTickAggregator* aggregator = nullptr;
};

void LayerTimingCallback(const char* tag, uint64_t duration_ns, void* user_data)
{
    auto* ctx = static_cast<ProfileContext*>(user_data);
    if (!ctx || !ctx->aggregator)
        return;
    ctx->aggregator->RecordLayer(tag, duration_ns);
}
...
int RunBenchmark(const char* model_path)
{
    ...
    BenchmarkInvokeProfile::TagTickAggregator profiler;
    ProfileContext profile_ctx{&profiler};

    std::printf("netkit MNIST MLP profile benchmark\n");
    ...
    std::printf("  profiler:    per-layer eval timing (benchmark-only build)\n");
    ...
    for (int run = 0; run < kRuns; ++run)
    {
        ...
        for (int i = 0; i < kMnistBenchmarkImageCount; ++i)
        {
            ...
            network->forward_timed(input, output, LayerTimingCallback, &profile_ctx);
    ...
    BenchmarkStats::PrintSummary("netkit", "mlp", "profile",
                                 BenchmarkStats::Compute(run_averages_us.data(), kRuns));
    profiler.PrintProfileSummaries("netkit", "mlp");
